...
^|C_G_C.2    ^|Remplir les champs dont on souhaite modifier les informations (Nom, Prénom, Email) et cliquer sur le bouton "Enregistrer" ^|Mise à jours des informations de l'utilisateur dans la base de données ^|Aucune ^| OK
...
^|ID ^|Action ^|Comportement attendu ^|Pré-conditions ^|Résultat
^|C_G_C.1    ^|Cliquer sur l'icône "panier" dans le menu. ^|La page visualiser_panier.php s'ouvre. ^| L'utilisateur doit être connecté. Le panier doit comporter au moins un produit. ^|OK.
^|C_G_C.2    ^|Cliquer sur l'îcone "cadeau" en dessous du prix total de la commande. ^|L'utilisateur est redirigé vers la page de visualisation du compte afin de pouvoir voir le nombre de points présents sur le compte et les utiliser. ^| Aucune ^|OK.
...
^|ID ^|Action ^|Comportement attendu ^|Pré-conditions ^|Résultat
^|C_G_A.1   ^|Cliquer sur le bouton "Donner un avis" ^|Apercevoir le champs de rédaction d'un avis s'afficher  ^|La page de détail d'un produit doit être ouverte et le client doit avoir commander le produit ^|OK
