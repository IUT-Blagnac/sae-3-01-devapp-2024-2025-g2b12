...
^|ID ^|Action ^|Comportement attendu ^|Pré-conditions ^|Résultat
^|C_G_C.1    ^|Sélectionner le bouton paramètre (L'îcone avec un stylo et une feuille) ^|Redirige vers la page de modification du compte de l'utilisateur ^|L'utilisateur doit se trouver sur la page de visualisation de son compte ... ^|
