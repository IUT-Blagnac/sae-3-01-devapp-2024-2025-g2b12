...
^|ID ^|Action ^|Comportement attendu ^|Pré-conditions ^|Résultat
^|C_G_C.1    ^|Sélectionner le bouton paramètre (L'îcone avec un stylo et une feuille) ^|Redirige vers la page de modification du compte de l'utilisateur ^|L'utilisateur doit se trouver sur la page de visualisation de son compte et être connecté ... ^|OK
...
^|C_G_C.3    ^|Remplir le champs avec le nombre de points souhaitant être utilisés par l'utilisateur et cliquer sur confirmer ^| Le nombre de points choisis ne peut pas être supérieur aux nombre de points sur le compte, ni négatif  ^|L'utilisateur est redirigé vers la page de visualisation de la commande avec le total comportant une réduction grâçe au nombre de points choisis par l'utilisateur ^|OK.
