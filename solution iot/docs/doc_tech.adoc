...
`resources` : Contient les fichiers et programmes externes nécessaires au fonctionnement de l'application Java.
    ...
    * `mqtt.py` : Le programme Python collecteur de données (client MQTT).
    * `configuration.ini` : Le fichier de configuration du programme Python.

`scr` : Contient le code source de l'application Java / JavaFX.
