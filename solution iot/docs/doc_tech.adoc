...
* *Exemples :*
    ...
    ** Entrée couremment traité dans une boucle de type `for-each` parcourant le contenu de dictionnaire → `m`.
...
* *Exemples :*
    ** Chaîne de caractères décrivant le préfixe d'un topic MQTT → `pTopicPrefix` (Cf. <<variables-recurrentes,Variables récurrentes>>).
    ** Liste de types de données → `pDataTypeList` (Cf. <<variables-recurrentes,Variables récurrentes>>).
    ** Composant graphique de type Bouton (`Button`) → `pButton` (Cf. <<variables-de-composants-graphiques,Variables de composants graphiques>>).
