...
==== *2.1.1 Description des sous-systèmes*

===== *_Application JavaFX_*
===== *_Programme Python_*

==== *2.1.2 Intéractions entre sous-systèmes*
. *Écriture du fichier de configuration par l'application Java*
	...
	* *À noter :* À cette étape, si un fichier de configuration existe déjà, celui-ci est remplacé par le fichier de configuration créé. Aucun mécanisme d'historisation ou de sauvegarde des fichiers de configurations n'a été mis en place.
...
. *Interruption du programme Python par l'application Java*
	* Lorsque le tableau de bord est fermé par l'utilisateur, le programme Python est automatiquement arrêté.
	* *À noter :* Après arrêt du processus de collecte de données, le fichier de configuration ainsi que les fichiers de données écrits ne sont pas effacés.
