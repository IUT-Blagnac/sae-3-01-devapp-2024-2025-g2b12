...
Les fichiers de données situés sous le répertoire `resources/data` sont des fichiers CSV permettant de stocker les données reçues des capteurs. Ces fichiers sont créés par le programme Python et lus par l'application Java.
...
Ce fichier est utilisé par l'application Java afin d'afficher dans le tableau de bord les données en temps réel pour chaque sujet observé.
...
Ce fichier est utilisé par l'application Java afin d'afficher les alerte en temps réel dans le tableau de bord.
...
Les fichiers dont le nom est de la forme `SUJET.csv` (où `SUJET` désigne le nom d'une salle ou `overview`) correspondent chacun à un fichier de données pour un sujet en particulier. Un fichier de ce type contient l'historique des données reçues pour un sujet.
...
----
{{ TYPE DE SUJET }};{{ TYPE DE DONNEE 1 }};{{ TYPE_DE DONNEE 2 }}
{{ SUJET }};{{ DONNÉE À L'INSTANT T0 }};{{ DONNÉE À L'INSTANT T0 }}
{{ SUJET }};{{ DONNÉE À L'INSTANT T1 }};{{ DONNÉE À L'INSTANT T1 }}
{{ SUJET }};{{ DONNÉE À L'INSTANT T2 }};{{ DONNÉE À L'INSTANT T2 }}
...
----
