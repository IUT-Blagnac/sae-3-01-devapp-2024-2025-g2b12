...
Le fichier `alert.csv` situé sous le répertoire `resources/data` correspond au fichier d'alertes. Il contient l'ensemble des alertes déclenchées par des dépassements de seuils. Une ligne de données du fichier correspond donc à une alerte pour un type de données et pour une salle.
...
Les fichiers situés sous le répertoire `resources/data` dont le nom est de la forme `[sujet].csv` où `[sujet]` désigne le nom d'un sujet correspond au fichier de données pour un sujet en particulier. Il contient l'historique des données reçues pour le sujet.
    
    * Dans le cas des capteurs AM107, ...
    * Dans le cas des capteurs SOLAREDGE, ...
