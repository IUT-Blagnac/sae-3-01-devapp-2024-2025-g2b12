...
`application` : Package "racine" de l'application.
    ...
    * `ApplicationMainFrame` : contrôleur de dialogue de la fenêtre principale de l'application (menu principal).
...
`application.styles` : Package des classes de stylisation de l'interface graphique.
    ...
    * `FontLoader` : Classe d'accès aux typographiques (fonts) utilisées dans l'interface graphique de l'application.

`application.thread` : Package des threads.

    * `CsvReaderTask` : Thread chargé de lire les fichiers de données CSV (fichiers `data.csv` et `alert.csv`)
...
`application.view` : Package des contrôleurs de vue (Cf. <<architecture-mvc,Architecture MVC>>).
    ...
    * `DataVisualisationPaneViewController` : Contrôleur de vue du tableau de bord (fenêtre de visualisation des données).
