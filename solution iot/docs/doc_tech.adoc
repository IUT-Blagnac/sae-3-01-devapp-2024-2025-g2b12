...
`application.thread` : Package des threads.
    ...
    * `CsvReaderTask` : Thread chargé de lire les fichiers de données `data.csv` (Cf. <<fichier-de-donnees-global,Fichier de données global>>) et `alert.csv` (Cf. <<fichier-d-alertes,Fichier d'alertes>>).
